...
### Added (T4.17 — v0.3 dataset_inputs writes are now opt-in)

- **Migration 014_dataset_inputs_v2.sql** creates `dataset_inputs_v2(id, run_id, dataset_id, tags_json)` with FK CASCADE on runs and datasets_v2. New primary link table for `mlflow.log_input(...)`.
- `LogInputs` now writes to `datasets_v2 + dataset_inputs_v2 + datasets` (the last is a thin metadata cache for MLflow wire shape — source_type/source/schema/profile fields don't fit datasets_v2). Writes to the v0.3 `dataset_inputs` + `dataset_input_tags` link tables are now gated by `LITEMLFLOW_ENABLE_DATASETS_V03_WRITES=1` (default off).
- `GetRunDatasets` reads from BOTH paths and deduplicates by (name, digest). Legacy runs that pre-date v2.1 keep working; opt-in operators see each input exactly once.
- `runDatasetEdges` (lineage) likewise reads both paths so the DAG keeps showing dataset edges for pre-v2.1 runs.
- New `mirrorIntoDatasetsV2Returning` returns the row id so the caller can link via dataset_inputs_v2 without a second SELECT.
- `LogInputs` now retries up to 6 times on transient SQLITE_BUSY_SNAPSHOT (517) — concurrent log_input calls with the same (workspace, name, digest) used to race on the SAVEPOINT existence-check path.

### Added (T4.18 partial — canonical baseline schema)

- **`internal/migrations/baseline/v2_baseline.sql`** — autogenerated consolidated CREATE TABLE/INDEX statements that produce the v2.1 schema from a fresh DB. Regenerate via `go run ./scripts/gen-baseline-schema > internal/migrations/baseline/v2_baseline.sql`.
- `TestSquashEquivalence` (new) applies `001..NN` to one DB and the baseline to another, asserts the schema dumps match. Any drift fails CI so the squash candidate stays accurate.
- **NOT YET activated**: the migration runner still applies `001..NN` sequentially on every install. The CLI subcommand that picks "baseline on fresh, 001..NN on stale" is v3.0 work; the v2.1 deliverable is the canonical baseline + drift guard.

### Deferred to v3.0

- **T4.20** tag-bag table collapse — `experiment_tags + tags + registered_model_tags + model_version_tags + dataset_input_tags → attributes(scope, scope_pk, key, value)` with per-scope FK triggers. The migration touches every parent-table delete path; the Storage architect's preferred option (1) needs the squash CLI from T4.18 to land first. ~80 Go LOC savings; deferred until the bigger migration boundary.
- The actual DROP of `dataset_inputs` + `dataset_input_tags` tables (T4.17 follow-up).
- The migration-squash CLI subcommand (T4.18 follow-up).

### Tests

`go test -count=1 -race ./...` — all 13 packages green. New: `TestSquashEquivalence`. Existing tests updated for the new dataset write path (TestLogInputs* now expect both v2 and v0.3 read paths to surface results identically).

### Operator notes

- The v0.3 `dataset_inputs` link table is no longer written by default. External tools that scrape it directly via SQL should either point at `dataset_inputs_v2` (joined to `datasets_v2`) or set `LITEMLFLOW_ENABLE_DATASETS_V03_WRITES=1`.
- The MLflow client wire shape (`runs/get` → `inputs.datasets[].source_type` etc.) is preserved via the always-on `datasets` metadata cache.
